...
**ClickHouse won on merit:** genuinely local, a real MergeTree engine with meaningful `ORDER BY`/`PARTITION BY`/skip-index design, and a large, reproducible margin over raw Spark SQL below.

### Benchmark results

Spark's one-time cluster/session startup (~9.57s) is excluded from the per-query numbers below (median of 3 runs each) since ClickHouse, as an always-on server, has no equivalent cost:
